...
def validate_create_input(request: CreateClientTokenInput) -> None:
    ...
    scopes = get_args(ClientScope)
    if (
        isinstance(request.scopes, (str, bytes))
        or not 1 <= len(request.scopes) <= len(scopes)
        or any(scope not in scopes for scope in request.scopes)
    ):
        raise JanuaryValidationError(f"scopes must contain 1-{len(scopes)} client-grantable scopes")
