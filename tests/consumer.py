...
class ConsumerTest(unittest.TestCase):
    def test_http_contract(self) -> None:
        ...
        try:
            ...
            client = January(max_retries=0, secret_key="sk-local-fixture", base_url=origin)
            token = client.client_tokens.create(
                end_user_id="user", scopes=["foods:read"], ttl_seconds=600
            )
            self.assertEqual(token.to_dict(), {"token": "ct-fixture", "expiresIn": 300})
            self.assertEqual(
                requests[0], {"end_user_id": "user", "scopes": ["foods:read"], "ttl_seconds": 600}
            )
