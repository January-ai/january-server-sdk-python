...
def run_example(script, service, directory, key=FAKE_KEY):
    ...
    return subprocess.run(
        [sys.executable, "-I", "-B", str(ROOT / "tests/example_harness.py"), script, service["url"]],
...
def test_quickstart_success(script, credentials, tmp_path):
    ...
    with local_service() as service:
        ...
        body["total_count"] = 1
        result = run_example(script, service, tmp_path, key=None if credentials == "dotenv" else FAKE_KEY)
...
@pytest.mark.parametrize("script", SCRIPTS)
@pytest.mark.parametrize("key", [None, "", "   "], ids=["no-file", "empty-file-key", "blank-file-key"])
def test_quickstart_missing_key_does_not_request(script, key, tmp_path):
    ...
    with local_service() as service:
        ...
        assert result.stderr == "Set JANUARY_API_KEY in .env or your environment before running this example.\n"
...
def test_quickstart_failure_is_nonzero_safe_and_not_retried(script, status, code, hint, tmp_path):
    with local_service() as service:
        service["response"] = {
            ...
            "body": {"code": code, "message": f"Server echoed {FAKE_KEY}",
                     "private": "private-echo-body", "token": "ct-private-echo-token"},
        }
        result = run_example(script, service, tmp_path)
        ...
        assert json.loads(lines[0].removeprefix("Food search failed: ")) == {
            "status": status, "code": code, "request_id": "req-offline-test",
        }
...
def load_example(script):
    spec = importlib.util.spec_from_file_location("quickstart_under_test", ROOT / "examples/quickstart" / script)
...
def test_quickstart_transport_diagnostics_do_not_print_cause(script, error_type, code, monkeypatch, capsys):
    ...
    with local_service() as service:
        ...
        monkeypatch.setattr(module, "January" if script == "main.py" else "AsyncJanuary", offline_failure)
        status = module.main() if script == "main.py" else asyncio.run(module.main())
        output = capsys.readouterr()
        assert status == 1
        assert output.out == ""
        assert output.err == f"Food search failed: {code}. Check connectivity; no automatic retry.\n"
...
@pytest.mark.parametrize("script", SCRIPTS)
@pytest.mark.parametrize("base_url", [None, "   ", "http://127.0.0.1:1", "https://unexpected.invalid"],
...
def test_default_base_url_without_opening_a_connection(script, base_url, monkeypatch, capsys):
    ...
    monkeypatch.setattr(module, "January" if script == "main.py" else "AsyncJanuary", no_network_client)
    status = module.main() if script == "main.py" else asyncio.run(module.main())
    assert status == 1
    assert constructors == [{"secret_key": FAKE_KEY}]
...
def test_public_docs_and_examples_do_not_expose_url_overrides():
    for relative in (
        "README.md", "CONTRIBUTING.md", "docs/live-testing.md", ".env.example",
        "examples/fastapi/README.md", "examples/fastapi/main.py",
        "examples/quickstart/main.py", "examples/quickstart/async_main.py",
...
def test_readme_onboarding_links_and_order():
    ...
    assert readme.index("## Before you begin") < readme.index("## Install\n")
    ...
    assert "python quickstart_async.py" in readme
    assert not re.search(r"unpublished|until publication|after publication|request SDK\s+access", readme, re.IGNORECASE)
    for url in (
