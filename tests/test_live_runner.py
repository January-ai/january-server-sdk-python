...
archive_spec = importlib.util.spec_from_file_location("january_distribution_check", SDK / "scripts/check-distribution.py")
...
PNG = base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jhQAAAABJRU5ErkJggg==")
...
def synthetic_environment(root):
    ...
    return {"JANUARY_API_KEY":"sk-offline-server",
            "JANUARY_E2E_USER_ID":"existing-user-must-be-ignored"}


@contextmanager
def service(fail=None, revoke_count=1, hide_logs=False):
    state = {"requests":[], "logs":{}, "tokens":{}, "users":set(), "fail":fail or {}, "revocations":[]}
    class Handler(BaseHTTPRequestHandler):
        def handle_request(self):
            from urllib.parse import urlsplit, parse_qs, unquote
            location = urlsplit(self.path)
            fixture = next(f for f in FIXTURES if f["method"] == self.command and re.fullmatch(re.sub(r"\{[^}]+\}", "[^/]+", f["path"]), location.path))
            ...
            state["requests"].append({"operation":op,"method":self.command,"query":parse_qs(location.query),"body":body,"user":user,"auth":auth})
            if user:
            ...
            if op == "listFoodLogs":
                response["body"] = {"total_count":len(state["logs"].get(user, {})), "items":list(state["logs"].get(user,{}).values())}
                if hide_logs:
            ...
            if op == "mintClientToken":
                ...
                response["body"].update(token=token, end_user_id=body["end_user_id"], scopes=body["scopes"],
                                         expires_in=300, expires_at=(datetime.now(timezone.utc)+timedelta(seconds=300)).isoformat())
            ...
            if op in state["fail"]:
                response = {"status":503,"headers":{"x-request-id":"req-safe"},
                            "body":{"code":"service_unavailable","docs_url":"https://example.com/docs","message":"sk-offline-server ct-do-not-print private meal"}}
...
def run(root, state, mode="both"):
    ...
    with loopback_http(state["url"]):
        code = live.main(["--mode",mode],root=root,environ=synthetic_environment(root),emit=output.append)
...
def test_env_loader_quotes_precedence_and_no_shell_evaluation(tmp_path):
    ...
    assert live.load_environment(tmp_path,{"JANUARY_ENV_FILE":"custom.env"})["JANUARY_API_KEY"] == "custom"
...
def test_env_loader_rejects_non_data_syntax(text):
    with pytest.raises(live.ConfigError): live.dotenv_values(text)


def test_missing_key_no_network_and_explicit_not_run(tmp_path,monkeypatch):
    def reject(*args,**kwargs): raise AssertionError("network is forbidden")
...
@pytest.mark.parametrize("legacy_url", [None, "http://127.0.0.1:1", "https://unexpected.invalid"],
...
def test_default_both_modes_all_18_local_http_and_live_ids(tmp_path, legacy_url):
    ...
    with service() as state:
        ...
        assert all(re.fullmatch(r"sdk-e2e-python-[0-9a-f-]{36}",u) and len(u)<=64 for u in state["revocations"])
        ...
        for request in state["requests"]:
            if request["operation"] in {"createFoodLog","predictGlucose"}:
                assert request["body"]["foods"] == [{"id":90000011,"serving":{"id":80000022,"quantity":1}}]
            if request["operation"] == "scanFoodPhoto":
                assert base64.b64decode(request["body"]["image"].split(",",1)[1]) == PNG
            if request["operation"] == "searchFoodsByNaturalLanguage": assert request["body"] == {"text":"one banana"}
...
def test_independent_operations_continue_and_dependencies_blocked(tmp_path):
    with service(fail={"searchFoods":True,"scanFoodPhoto":True,"searchFoodsByNaturalLanguage":True}) as state:
...
def test_cleanup_after_ambiguous_mint_and_update_failure(tmp_path,mode):
    with service(fail={"mintClientToken":True,"updateFoodLog":True}) as state:
        code,report,output = run(tmp_path,state,mode)
        assert code == 1
        assert not state["tokens"]
        assert not any(state["logs"].values())
...
def test_package_archive_guard_rejects_secret_and_results_paths(tmp_path,member,kind):
    ...
    if kind=="wheel":
        with zipfile.ZipFile(artifact,"w") as archive: archive.writestr(member,"synthetic-data")
    else:
        with tarfile.open(artifact,"w:gz") as archive:
            info=tarfile.TarInfo(member); info.size=1
            archive.addfile(info,io.BytesIO(b"x"))
    with pytest.raises(ValueError): archives.check_archive(str(artifact))


def test_package_archive_guard_accepts_public_code(tmp_path):
    artifact=tmp_path/"test.whl"
    with zipfile.ZipFile(artifact,"w") as archive: archive.writestr("januaryai/client.py","# test")
