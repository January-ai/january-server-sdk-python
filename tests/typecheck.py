from januaryai import AsyncJanuary, ClientToken, January
from typing_extensions import assert_type
from januaryai import FoodPortion, FoodPortionError, FoodPortionErrorCode, ResponseMetadata, models
...
def check_sync(client: January) -> None:
    ...
    assert_type(user.food_logs.create(foods=[{"id": 12, "serving": {"id": 5, "quantity": 1}}]), models.FoodLog)
...
async def check_async(client: AsyncJanuary) -> None:
    ...
    assert_type(await client.for_user("user").food_logs.create(foods=[portion.selection]), models.FoodLog)
