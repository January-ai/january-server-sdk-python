...
import httpx
import pytest
from januaryai import (January, AsyncJanuary, JanuaryAPIError, JanuaryCancelledError,
                       JanuaryConfigurationError, JanuaryTimeoutError, JanuaryValidationError,
                       JanuaryResponseError, ResponseMetadata, models)
...
def test_all_operations(fixture, async_mode):
    with local_service() as service:
        ...
        if fixture["operationId"] == "revokeClientTokens":
            ...
            assert result.model_dump(by_alias=True, exclude_unset=True) == fixture["response"]["body"]
    ...
    with local_service() as service:
        ...
        with pytest.raises(JanuaryAPIError) as caught:
            if async_mode: asyncio.run(run())
                ...
                with January(secret_key="sk-local-fixture", base_url=service["url"]) as client: client.credits()
        e = caught.value
...
def test_root_surface_and_immutable_user_views():
    with local_service() as service:
        ...
        with January(secret_key="sk-local-fixture", base_url=service["url"]) as client:
            ...
            with pytest.raises(FrozenInstanceError): a._context = {}
            with pytest.raises(TypeError): a._context["end_user_id"] = "bob"
            a.foods.search(query="eggs")
            b.foods.search(query="eggs")
            client.foods.search(query="eggs")
            assert [r["headers"].get("x-end-user-id") for r in service["requests"]] == ["alice","bob",None]
...
def test_async_user_context_is_concurrency_safe():
    with local_service() as service:
        ...
        async def run():
            async with AsyncJanuary(secret_key="sk-local-fixture", base_url=service["url"]) as client:
                await asyncio.gather(*(client.for_user(str(i)).foods.search(query="eggs") for i in range(10)))
        asyncio.run(run())
        assert {r["headers"]["x-end-user-id"] for r in service["requests"]} == {str(i) for i in range(10)}


def test_single_revoke_even_at_500_and_encoding():
    fixture = deepcopy(next(f for f in FIXTURES["operations"] if f["operationId"] == "revokeClientTokens"))
...
def test_path_encoding_and_date_serialization():
    ...
    with local_service() as service:
        service["response"] = fixture["response"]
        with January(secret_key="sk-local-fixture", base_url=service["url"]) as client:
            client.for_user("user", end_user_timezone="America/New_York").food_logs.update(log_id="a/b ?#", name="Lunch")
        request = service["requests"][0]
...
def test_forward_compatible_responses_and_redaction():
    fixture = deepcopy(next(f for f in FIXTURES["operations"] if f["operationId"] == "mintClientToken"))
    ...
    with local_service() as service:
        ...
        with January(secret_key="sk-local-fixture", base_url=service["url"]) as client:
            ...
            service["response"] = {"status":400,"headers":{},"body":{"code":"bad_input","docs_url":"https://example.com/docs","message":"sk-local-fixture private-user private meal"}}
            with pytest.raises(JanuaryAPIError) as caught:
                client.food_analysis.analyze_description(query="private meal", end_user_id="private-user")
...
def test_short_query_preserves_machine_metadata_and_filters_credentials():
    response = {"status":429,"headers":{"x-request-id":"request-abc","x-api-key":"hidden", "X-Token":"hidden", "X-Client-Token":"hidden", "Set-Cookie":"hidden", "Retry-After":"2"},
                "body":{"code":"rate_limited","docs_url":"https://partners.january.ai/v1.2/docs","message":"Rate limit exceeded"}}
    with local_service() as service:
        service["response"] = response
        with January(secret_key="sk-local-fixture", base_url=service["url"]) as client:
            with pytest.raises(JanuaryAPIError) as caught: client.foods.search(query="a")
        error = caught.value
        assert error.code == "rate_limited"
        assert error.docs_url == response["body"]["docs_url"]
        assert error.request_id == "request-abc"
        assert error.response.retry_after == "2"
        assert not any("token" in k or "api-key" in k or "cookie" in k for k in error.response.headers)


def test_unset_null_and_typed_models():
    contract = Contract()
    assert contract.encode({"value":None}, {"type":"object","properties":{"value":{"type":"string","nullable":True}}}, "request") == {"value":None}
    assert contract.encode({}, {"type":"object","properties":{"value":{"type":"string","nullable":True}}}, "request") == {}
    assert models.FoodLog(id="log", foods=[], timestamp_utc="opaque").model_dump(exclude_unset=True) == {"id":"log", "foods":[],"timestamp_utc":"opaque"}
    fixture = next(f for f in FIXTURES["operations"] if f["operationId"] == "createFoodLog")
    with local_service() as service:
        service["response"] = fixture["response"]
        with January(secret_key="sk-local-fixture", base_url=service["url"]) as client:
            client.for_user("user").food_logs.create(foods=[models.FoodLogInputFood(id=12,serving=models.FoodLogInputServing(id=5,quantity=1))], timestamp_utc=datetime(2026,8,30,tzinfo=timezone.utc))
            assert service["requests"][-1]["body"] == {"foods":[{"id":12,"serving":{"id":5,"quantity":1}}],"timestamp_utc":"2026-08-30T00:00:00Z"}
            with pytest.raises(JanuaryValidationError): client.for_user("user").food_logs.update(log_id="log", name=None)
            assert len(service["requests"]) == 1


@pytest.mark.parametrize("key", ["", "ct-client-token", "wrong-key", "sk-key\n"])
def test_reject_invalid_secret(key):
    with pytest.raises(JanuaryConfigurationError): January(secret_key=key)


@pytest.mark.parametrize("timeout", [0, -1, float("inf"), float("nan")])
def test_reject_unbounded_timeout(timeout):
    with pytest.raises(JanuaryConfigurationError): January(secret_key="sk-local-fixture", timeout=timeout)


def test_timeout_cancellation_and_no_retry():
    with local_service() as service:
        service["delay"] = .1
        service["response"] = next(f for f in FIXTURES["operations"] if f["operationId"]=="credits")["response"]
        with January(secret_key="sk-local-fixture", base_url=service["url"]) as client:
            with pytest.raises(JanuaryTimeoutError): client.credits(timeout=.01)
            event = Event(); event.set()
            with pytest.raises(JanuaryCancelledError): client.credits(cancel_event=event)
        async def run():
            async with AsyncJanuary(secret_key="sk-local-fixture", base_url=service["url"]) as client:
                with pytest.raises(JanuaryTimeoutError): await client.credits(timeout=.01)
...
def test_redirects_not_followed_and_injected_transport_not_closed():
    ...
    with httpx.Client(transport=httpx.MockTransport(handle), follow_redirects=True) as transport:
        with January(secret_key="sk-local-fixture", http_client=transport) as client:
            with pytest.raises(JanuaryAPIError): client.credits()
...
def test_malformed_response_and_validation_before_network():
    with local_service() as service:
        with January(secret_key="sk-local-fixture", base_url=service["url"]) as client:
            with pytest.raises(JanuaryResponseError): client.credits()
            with pytest.raises(JanuaryValidationError): client.mint_client_token(end_user_id="user", ttl_seconds=299)
            with pytest.raises(JanuaryValidationError): client.foods.lookup_barcode(upc="nope")
            with pytest.raises(JanuaryValidationError): client.food_logs.list(start="2026-08-01", end="2026-08-30")
            assert len(service["requests"])==1
...
def test_manifest_matches_public_methods():
    ...
    for client_type in [January, AsyncJanuary]:
        ...
        if client_type is AsyncJanuary: asyncio.run(client.close())
        else: client.close()
