...
from januaryai.models import (FoodLogInputFood, FoodSearchItem, NutrientAmount,
                              NutritionFacts, ServingOption)
from installed_consumer import FIXTURES, arguments, local_service


NUTRIENTS = (
    "calories", "protein", "carbohydrates", "net_carbohydrates", "total_fat",
    "trans_fat", "saturated_fat", "fiber", "total_sugars", "added_sugars",
    "cholesterol", "calcium", "iron", "potassium", "sodium", "vitamin_d",
)


def example_food():
    return FoodSearchItem(
        id=42, name="Test food", glycemic_index=50, glycemic_load=8,
        nutrients=NutritionFacts(calories=NutrientAmount(value=100, unit="cal"),
                                 protein=NutrientAmount(value=10, unit="g")),
        servings=[
            ServingOption(id=1, quantity=1, unit="slice", scaling_factor=1, weight_grams=50, is_primary=True),
            ServingOption(id=2, quantity=2, unit="pieces", scaling_factor=3, weight_grams=120, is_primary=False),
        ],
    )
...
def test_gram_serving_matches_ios_client_scaling():
    food = FoodSearchItem(
        id=70381819, name="banana", glycemic_index=51, glycemic_load=12,
        ...
        servings=[ServingOption(id=2, quantity=100, unit="g", scaling_factor=.8474576271,
                                weight_grams=100, is_primary=False)],
    )
    portion = FoodPortion.from_food(food, serving_id=2, quantity=200)
    assert portion.nutrition.calories.value == pytest.approx(178, abs=.001)
    assert portion.nutrition.protein.value == pytest.approx(2.18, abs=.001)
    assert portion.nutrition.carbohydrates.value == pytest.approx(45.68, abs=.001)
    assert portion.nutrition.potassium.value == pytest.approx(715.254, abs=.001)
    assert portion.nutrition.potassium.unit == "mg"
    assert portion.total_weight_grams == 200
    assert portion.glycemic_index == 51
    assert portion.glycemic_load == pytest.approx(20.3389, abs=.001)


def test_scales_all_16_generated_nutrients_and_preserves_units():
    assert set(NUTRIENTS) == set(NutritionFacts.model_fields)
    raw = {name:{"value":index + .5, "unit":f"unit-{index}"} for index,name in enumerate(NUTRIENTS)}
...
def test_sparse_zero_and_empty_nutrients_keep_presence():
    food = example_food().model_copy(update={"nutrients":NutritionFacts(protein=NutrientAmount(value=0, unit="g"))})
    portion = FoodPortion.from_food(food, quantity=2)
    assert portion.nutrition.model_fields_set == {"protein"}
    assert portion.nutrition.model_dump(by_alias=True, exclude_unset=True) == {"protein":{"value":0,"unit":"g"}}
...
def test_existing_explicit_none_and_missing_weight_glycemic_values_are_preserved():
    food = FoodSearchItem(id=42, name="Test", nutrients=NutritionFacts(protein=None),
                          servings=[ServingOption(id=1, quantity=2, unit="pieces", scaling_factor=1,
                                                  weight_grams=None, is_primary=True)])
...
def test_defaults_to_selected_serving_quantity_and_first_primary_or_first():
    ...
    both_primary = [serving.model_copy(update={"is_primary":True}) for serving in reversed(food.servings)]
    assert FoodPortion.from_food(food.model_copy(update={"servings":both_primary})).serving.id == 2
    no_primary = [serving.model_copy(update={"is_primary":False}) for serving in reversed(food.servings)]
    ...
    assert FoodPortion.from_food(food.model_copy(update={"servings":later_primary})).serving.id == 1


@pytest.mark.parametrize("quantity",[0,-1,10_001,float("nan"),float("inf"),-float("inf"),True,"2"])
...
def test_quantity_boundary_and_fraction():
    ...
    assert FoodPortion.from_food(example_food(), quantity=.25).nutrition.calories.value == 25
...
def test_does_not_mutate_or_share_mutable_model_state():
    ...
    assert food.nutrients.model_fields_set == before_fields
    assert food.model_dump(by_alias=True, exclude_unset=True) == before
    with pytest.raises(FrozenInstanceError): portion.quantity = 99
...
def test_portion_selection_works_in_sdk_log_and_glucose_requests(async_mode):
    ...
    with local_service() as service:
        async def run():
            client = (AsyncJanuary if async_mode else January)(secret_key="sk-local-fixture",base_url=service["url"])
            ...
            try:
                ...
                fixture = next(f for f in FIXTURES["operations"] if f["operationId"]=="predictGlucose")
                service["response"] = fixture["response"]
        ...
        assert all(request["body"]["foods"] == [{"id":42,"serving":{"id":2,"quantity":4}}] for request in service["requests"])
