...
import httpx
from januaryai import AsyncJanuary, January, JanuaryAPIError, JanuaryTimeoutError, UNSET


ROOT = Path(__file__).resolve().parents[2]
OPERATIONS = (
    "credits", "foods.search", "foods.autocomplete", "foods.get",
    "foods.lookup_barcode", "foods.suggest_alternatives", "restaurants.search",
    "restaurants.search_menu_items", "food_analysis.analyze_photo",
    "food_analysis.analyze_description", "food_analysis.correct", "food_logs.create",
    "food_logs.list", "food_logs.update", "food_logs.delete", "glucose.predict",
    "mint_client_token", "revoke_client_tokens",
)
...
class Config:
    ...
    def load(cls, root: Path, environ: Mapping[str, str]) -> Config:
        ...
        try:
            ...
            if not math.isfinite(timeout) or timeout <= 0 or not -90 <= latitude <= 90 or not -180 <= longitude <= 180:
                raise ValueError()
        except ValueError:
        ...
        if not query.strip() or len(query) > 256 or not restaurant.strip() or len(restaurant) > 256 or not re.fullmatch(r"[0-9]{6,14}", upc):
        ...
        mime = "image/png" if image.startswith(b"\x89PNG\r\n\x1a\n") else "image/jpeg" if image.startswith(b"\xff\xd8\xff") else None
        if mime is None or len(image) > 3_500_000:
            raise ConfigError("image_fixture_invalid")
        return cls(key, timeout, upc, query, restaurant, latitude, longitude,
                   f"data:{mime};base64,{base64.b64encode(image).decode('ascii')}")


class Reporter:
    def __init__(self, modes: list[str], emit: Callable[[str], None]) -> None:
        self.modes = modes
        self.emit = emit
    ...
    def safe(self, value: Any) -> str | None:
        if not isinstance(value, str) or not re.fullmatch(r"[A-Za-z0-9_.:/-]{1,128}", value):
            return None
        if any(secret and secret in value for secret in self.secrets) or re.search(r"(?:sk|ct)-", value):
            return "redacted"
        return value

    def record(self, mode: str, operation: str, status: str, *, kind: str = "operation",
               code: str | None = None, request_id: str | None = None, http_status: int | None = None,
               duration: float = 0, cleanup: bool = False, reason: str | None = None) -> dict[str, Any]:
        row = {"mode":mode, "operation":operation, "kind":kind, "status":status,
               "code":self.safe(code), "requestId":self.safe(request_id), "httpStatus":http_status,
               "durationMs":round(duration * 1000, 2), "cleanup":cleanup}
        if reason is not None:
            row["reason"] = reason  # All callers supply static dependency labels only.
        self.results.append(row)
        self.emit(json.dumps({k:row[k] for k in ("mode", "operation", "status", "code", "requestId")}))
        return row

    def remaining(self, reason: str) -> None:
        for mode in self.modes:
            done = {r["operation"] for r in self.results if r["mode"] == mode and r["kind"] == "operation"}
    ...
    def document(self) -> dict[str, Any]:
        ...
        counts = {status:sum(r["status"] == status for r in operations) for status in ("PASS", "FAIL", "BLOCKED")}
        cleanup_failures = sum(r["cleanup"] and r["status"] != "PASS" for r in self.results)
        passed = (len(operations) == 18 * len(self.modes) and counts["PASS"] == len(operations)
                  and all(r["status"] == "PASS" for r in self.results))
        return {"language":"python", "modes":self.modes, "status":"PASS" if passed else "FAIL",
                "expectedOperations":18 * len(self.modes), "counts":counts,
                "cleanupFailures":cleanup_failures, "durationMs":round((time.monotonic()-self.started)*1000, 2),
...
async def token_probe(config: Config, token: str, mode: str) -> Any:
    ...
    operation = next(o for o in projection["operations"].values() if o["resource"] == "foods" and o["publicMethod"] == "search")
    kwargs = {"method":operation["method"], "url":"https://partners.january.ai" + operation["path"],
              "params":{"query":config.query, "limit":1},
              "headers":{"Authorization":f"Bearer {token}", "Accept":"application/json"}}
    if mode == "async":
        async with httpx.AsyncClient(timeout=config.timeout, follow_redirects=False, trust_env=False) as http:
    ...
    if response.status_code != 200:
        try:
            ...
            payload = {}
        raise JanuaryAPIError("Client-token verification failed", status_code=response.status_code,
...
async def workflow(config: Config, mode: str, report: Reporter) -> None:
    ...
    client = (AsyncJanuary if mode == "async" else January)(secret_key=config.api_key,
    ...
    async def step(label: str, fn: Callable[[], Any], *, validate: Callable[[Any], None] | None = None,
                   blocked: str | None = None, kind: str = "operation", cleanup: bool = False) -> Any:
        if blocked:
            report.record(mode, label, "BLOCKED", kind=kind, code="dependency_failed", reason=blocked, cleanup=cleanup)
            return None
        ...
        try:
            ...
            code = error.args[0] if isinstance(error, CheckFailed) else "timeout" if isinstance(error, (JanuaryTimeoutError, httpx.TimeoutException)) else error.code if isinstance(error, JanuaryAPIError) else "runner_error"
            report.record(mode, label, "FAIL", kind=kind, code=code, request_id=request_id,
                          http_status=status, duration=time.monotonic()-clock, cleanup=cleanup)
            return None
        status, request_id = metadata(value)
        report.record(mode, label, "PASS", kind=kind, request_id=request_id,
                      http_status=status, duration=time.monotonic()-clock, cleanup=cleanup)
        return value

    try:
        await step("credits", lambda:client.credits())
        found = await step("foods.search", lambda:user.foods.search(query=config.query),
        ...
        food = await step("foods.get", lambda:user.foods.get(food_id=food_id),
                          blocked="foods.search did not return a food" if food_id is None else None)
        await step("foods.lookup_barcode", lambda:user.foods.lookup_barcode(upc=config.upc),
                   validate=lambda r:require(bool(r.items), "barcode_not_found"))
        await step("foods.suggest_alternatives", lambda:user.foods.suggest_alternatives(food_id=food_id),
                   blocked="foods.search did not return a food" if food_id is None else None)
        location = {"query":config.restaurant_query, "latitude":config.latitude, "longitude":config.longitude}
        await step("restaurants.search", lambda:user.restaurants.search(**location))
        await step("restaurants.search_menu_items", lambda:user.restaurants.search_menu_items(**location))
        photo = await step("food_analysis.analyze_photo", lambda:user.food_analysis.analyze_photo(image=config.image),
                           validate=lambda r:require(bool(r.detections), "no_photo_detections"))
        description = await step("food_analysis.analyze_description", lambda:user.food_analysis.analyze_description(query="one banana"),
                                 validate=lambda r:require(bool(r.detections), "no_description_detections"))
        analysis = photo or description
        await step("food_analysis.correct", lambda:user.food_analysis.correct(
            meal_name=analysis.meal_name if analysis.meal_name is not None else UNSET,
            detections=analysis.detections, user_input="The portion is one serving."),
        ...
        selection = [{"id":food.id, "serving":{"id":serving.id, "quantity":1}}] if serving is not None else None
        ...
        async def create_log() -> Any:
            ...
            log = await invoke(user.food_logs.create, foods=selection, timestamp_utc=started, name=marker)
        ...
        created = await step("food_logs.create", create_log,
        ...
        def check_logs(logs: Any) -> None:
            ...
            if created is not None:
                require(any(log.id == created.id for log in logs.items), "created_log_missing_from_list")

        await step("food_logs.list", lambda:user.food_logs.list(**date_range), validate=check_logs)
        await step("food_logs.update", lambda:user.food_logs.update(log_id=created.id, name=marker + " updated"),
                   blocked="food_logs.create did not return a log" if created is None else None,
                   validate=lambda r:require(r.id == created.id, "updated_log_id_mismatch"))
        await step("glucose.predict", lambda:user.glucose.predict(
            user_profile={"age":35, "sex":"female", "height":{"value":165, "unit":"cm"},
                          "weight":{"value":65, "unit":"kg"}, "activity_level":"moderately_active"},
            foods=selection, start_time=started),
            blocked="foods.get did not return a usable serving" if selection is None else None,
        ...
        async def mint_token() -> Any:
            ...
            token = await invoke(client.mint_client_token, end_user_id=user_id, scopes=["foods:read"], ttl_seconds=300)
            ...
            require(math.isfinite(token.expires_in) and 0 < token.expires_in <= 300, "token_expiry_invalid")
            try:
        ...
        await step("client_token.foods.search", lambda:token_probe(config, token.token, mode), kind="check",
            ...
            if create_attempted and not create_acknowledged and not owned_logs:
                def recover(logs: Any) -> None:
                    for log in logs.items:
                        if log.name == marker and log.id:
                            owned_logs.add(log.id)
                await step("cleanup.food_logs.list", lambda:user.food_logs.list(**date_range),
                           validate=recover, kind="cleanup", cleanup=True)
                if not owned_logs:
                    await step("cleanup.food_logs.confirm", lambda:require(False, "ambiguous_create_cleanup_unconfirmed"),
                               kind="cleanup", cleanup=True)
            ids = sorted(owned_logs)
            if not ids:
                await step("food_logs.delete", lambda:None, blocked="no run-owned log ID available")
            for index, log_id in enumerate(ids):
                deleted = await step("food_logs.delete" if index == 0 else "cleanup.food_logs.delete",
                                     lambda log_id=log_id:user.food_logs.delete(log_id=log_id),
                                     kind="operation" if index == 0 else "cleanup", cleanup=True,
                                     validate=lambda r:require(r.status == "deleted", "log_delete_not_confirmed"))
                if deleted is not None:
                    owned_logs.discard(log_id)
        finally:
            try:
                await step("revoke_client_tokens", lambda:client.revoke_client_tokens(end_user_id=user_id),
                           cleanup=True, blocked="mint_client_token was not attempted" if not mint_attempted else None,
                           validate=lambda r:require(r.status_code == 204 and r.revoked_count is not None, "revocation_not_confirmed"))
            finally:
                await invoke(client.close)


def main(argv: list[str] | None = None, *, root: Path = ROOT,
         environ: Mapping[str, str] | None = None, emit: Callable[[str], None] = print) -> int:
    parser = argparse.ArgumentParser(description="Explicit opt-in live API demo: creates synthetic data and consumes credits.")
    parser.add_argument("--mode", choices=("sync", "async", "both"), default="both")
    args = parser.parse_args(argv)
    modes = ["sync", "async"] if args.mode == "both" else [args.mode]
    report = Reporter(modes, emit)
    exit_code = 1
    try:
        ...
        async def run() -> None:
            for mode in modes:
                try:
                    await workflow(config, mode, report)
                except Exception:
    ...
    try:
        ...
        emit(json.dumps({"operation":"report", "status":"FAIL", "code":"report_write_failed", "requestId":None}))
        return 1
    emit(json.dumps({"operation":"summary", "status":document["status"], "counts":document["counts"],
